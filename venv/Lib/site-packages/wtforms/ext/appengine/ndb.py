...
way to create validators based on ``ndb.Model`` classes. No malabarism or black
...
to create other validators using ``model_form()``.
...
The class returned by ``model_form()`` can be used as a base class for validators
mixing non-model fields and/or other model validators. For example:
...
def model_form(model, base_class=Form, only=None, exclude=None, field_args=None,
    ...
    subforms with other model validators, among other possibilities.
